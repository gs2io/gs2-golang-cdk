...
func (p *CurrentMasterData) ResourceName() string {
	return "LoginReward_CurrentBonusMaster_" + p.namespaceName
}
...
func (p *CurrentMasterData) Properties() map[string]interface{} {
	bonusModels := make([]map[string]interface{}, len(p.bonusModels))
	for i, item := range p.bonusModels {
		bonusModels[i] = item.Properties()
	}
