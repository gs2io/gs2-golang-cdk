...
func (p *CurrentMasterData) ResourceName() string {
	return "Guild_CurrentGuildMaster_" + p.namespaceName
}
...
func (p *CurrentMasterData) Properties() map[string]interface{} {
	guildModels := make([]map[string]interface{}, len(p.guildModels))
	for i, item := range p.guildModels {
		guildModels[i] = item.Properties()
	}
