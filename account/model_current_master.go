...
func NewCurrentMasterData(
	...
	self := CurrentMasterData{
		...
		takeOverTypeModels: takeOverTypeModels,
	}
...
func (p *CurrentMasterData) ResourceName() string {
	return "Account_CurrentModelMaster_" + p.namespaceName
}
...
func (p *CurrentMasterData) Properties() map[string]interface{} {
	takeOverTypeModels := make([]map[string]interface{}, len(p.takeOverTypeModels))
	for i, item := range p.takeOverTypeModels {
		takeOverTypeModels[i] = item.Properties()
	}
